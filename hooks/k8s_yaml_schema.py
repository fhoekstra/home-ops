...
def _split_yaml_documents(text: str) -> list[tuple[str, str]]:
    ...
    for m in DOC_MARKER_RE.finditer(text):
        line_end = (
            m.end() + 1 if m.end() < len(text) and text[m.end()] == "\n" else m.end()
        )
...
def _is_k8s_resource(obj: Any) -> bool:
    return (
        isinstance(obj, dict) and bool(obj.get("apiVersion")) and bool(obj.get("kind"))
    )
...
def _match_rule(
    ...
    if (
        "file_regex" in match
        and re.search(str(match["file_regex"]), file_posix) is None
    ):
        return False
    ...
    if "jmespath" in match:
        ...
        inline = {
            k: match[k]
            for k in ("exists", "equals", "one_of", "match_regex")
            if k in match
        }
